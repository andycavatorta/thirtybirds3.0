...
class Controllers(threading.Thread):
    ...
    def collect_boards(self, mcu_serial_device_path, resp_str):
        found = True
        board_names_in_config = list(self.boards_config.keys())
        board_names_found = list(self.boards.keys())
        print(board_names_in_config, board_names_found)
        #for board_name, board in self.boards.values():
