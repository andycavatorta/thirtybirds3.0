...
# Initialize TLC5947
DRIVER_COUNT = 4  # change this to the number of drivers you have chained
...
def first_last():
    ...
    redB   = tlc5947.create_pwm_out( 1 * 24 + 0 )
    greenB = tlc5947.create_pwm_out( 1 * 24 + 1 )
    blueB  = tlc5947.create_pwm_out( 1 * 24 + 2 )

    redC   = tlc5947.create_pwm_out( 2 * 24 + 0 )
    greenC = tlc5947.create_pwm_out( 2 * 24 + 1 )
    blueC  = tlc5947.create_pwm_out( 2 * 24 + 2 )

    redD   = tlc5947.create_pwm_out( 3 * 24 + 0 )
    greenD = tlc5947.create_pwm_out( 3 * 24 + 1 )
    blueD  = tlc5947.create_pwm_out( 3 * 24 + 2 )
    ...
    while True:
        for (pinR, pinG, pinB) in ((redA, greenA, blueA), (redB, blueB, greenB) , (redC, blueC, greenC), (redD, greenD, blueD) ):
